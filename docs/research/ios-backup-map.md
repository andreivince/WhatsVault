...
- `cargo run -p whatsvault-proof` provides a private-safe report for real backup checks, including aggregate WhatsApp database counts and bounded chat/message samples when readable
...
- a bounded real chat-list sample can be read
- a bounded real chat sample can be imported into the normalized app model
- chat import proof exposes aggregate counts, sampled counts, and sample limits only
...
The first real MobileSync backup proof gate is complete: WhatsVault can locate WhatsApp files through `Manifest.db`, resolve `ChatStorage.sqlite` to the physical backup file, read aggregate counts, read a bounded chat-list sample, import a bounded real chat sample into the normalized app model, render that chat in the packaged desktop UI, preview bounded media from the backup, and export a bounded self-contained HTML file without manual SQLite copying.
