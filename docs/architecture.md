...
- Desktop commands are asynchronous, and blocking filesystem, ZIP, SQLite, media, and export work
  ...
  This includes native dialog waits and backup metadata/manifest preparation. The source registry
  lock is held only for handle registration, never while reading backup metadata.
...
- `components/MessageTimeline.tsx` uses TanStack Virtual to measure variable-height rows, preserve message anchors when earlier messages are added, and keep the latest message visible on opening a chat. Message IDs own measurement identity; media and pane resizing update measurements. The virtualizer owns scroll compensation, so browser scroll anchoring is disabled for this pane. `MessageBubble.tsx` owns message and attachment presentation.
